...
const event = document.addEventListener('DOMContentLoaded', initTodos);
...
function renderTodo(data) {
  ...
  divTag.textContent = data.content;

  const buttonDone = document.createElement('button');
  buttonDone.textContent = '완료';
  ...
  liTag.append(divTag, buttonDone, buttonDelete);
...
async function addTodo() {
  // 입력 필드에서 새로운 Todo의 내용을 가져와 서버에 저장하고, 해당 요소를 목록에 추가한다. (await)
  const response = await fetch(URL, {
    method: 'POST',
    body: JSON.stringify({
